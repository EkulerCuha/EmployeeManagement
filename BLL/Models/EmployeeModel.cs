...
public class EmployeeModel
{
    ...
    public string Projects => string.Join("<br>", Record.EmployeeProjects?.Select(ep => ep.Project?.Name));

    [DisplayName("Projects")]
    public List<int> ProjectIds
    {
        get => Record.EmployeeProjects?.Select(ep => ep.ProjectId).ToList();
        set => Record.EmployeeProjects = value.Select(v => new EmployeeProject { ProjectId = v }).ToList();
    }
